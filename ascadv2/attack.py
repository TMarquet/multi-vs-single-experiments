...
from train_models import model_single_task_softmax_check, model_single_task_whole , model_single_task_extracted , model_single_task_flat,  model_single_task_xor_whole , model_single_task_xor_extracted , model_single_task_xor_flat   , model_multi_task_whole, model_multi_task_extracted, model_multi_task_flat
...
class Attack:
    def __init__(self,training_type,convolution_blocks , kernel_size,filters , pooling_size,dense_blocks,dense_units,n_experiments = 1000,n_traces = 50000,model_type = 'multi_task'):
        ...
        self.alpha = get_hot_encode(np.array(labels_dict['alpha'],dtype = np.uint8)[:self.n_traces])
        ...
        if model_type == 'multi_task':
            multi_name = '{}_{}_{}.h5'.format('model_{}_{}'.format(model_type,training_type), 'all',id_model) 
            X_multi = {}
            ...
            for byte in range(16):
                name = '{}_{}_{}.h5'.format('model_{}'.format(model_type),byte ,id_model) 
                X_single = {} 

                if training_type == 'extracted':
                    model_struct = model_single_task_xor_extracted(convolution_blocks ,dense_blocks, kernel_size,filters , pooling_size,dense_units,summary = False) if 'xor' in model_type else model_single_task_extracted(convolution_blocks ,dense_blocks, kernel_size,filters , pooling_size,dense_units,summary = False) 
                    ...
                    X_single['inputs_rin'] = traces[:,4088:4088+1605]                   
                      ...
                      model_struct = model_single_task_xor_flat(convolution_blocks ,dense_blocks, kernel_size,filters , pooling_size,dense_units,summary = False) if 'xor' in model_type else model_single_task_flat(convolution_blocks ,dense_blocks, kernel_size,filters , pooling_size,dense_units,summary = False) 
                      X_single['inputs_intermediate'] = traces[:,4088+1605 :4088+1605 +93*16]        
                      X_single['alpha'] = self.alpha
                      X_single['inputs_rin'] = traces[:,4088:4088+1605] 
                elif training_type == 'whole':
                    model_struct = model_single_task_xor_whole(convolution_blocks ,dense_blocks, kernel_size,filters , pooling_size,dense_units,summary = False) if 'xor' in model_type else model_single_task_whole(convolution_blocks ,dense_blocks, kernel_size,filters , pooling_size,dense_units,summary = False)        
                    X_single['alpha'] = self.alpha
                    X_single['traces'] = traces[:,4088:4088+1605+93*16] 
                
                elif training_type == 'check_softmax':
                    model_struct = model_single_task_softmax_check(convolution_blocks ,dense_blocks, kernel_size,filters , pooling_size,dense_units,summary = False) 
                    X_single['inputs_intermediate'] = traces[:,4088+1605 +93*byte:4088+1605 +93*(byte+1)]        
                    X_single['alpha'] = self.alpha
                    X_single['inputs_rin'] = traces[:,4088:4088+1605] 
                else:
...
if __name__ == "__main__":
    ...
    for model_type in MODEL_TYPE:
        for model_name in os.listdir(MODEL_FOLDER):
            ...
            format_string = multi_task + '_cb{}ks{}f{}ps{}db{}du{}.h5'
